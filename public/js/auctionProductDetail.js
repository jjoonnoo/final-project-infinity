...
function auctionProductDetail() {
  $.ajax({
      ...
      success: function (response) {
          ...
          let temp_html_btn = `
          <a href="/product/auction_purchase/${auction_product_id}"><button id="btn1">즉시 구매하기</button></a>
                    `
...
function bidBtn() {
  ...
  if (compare_price === product_buy_now_price) {
    if(!confirm('즉시 구매가와 가격이 같습니다. 즉시 구매 하시겠습니까?')) {
      return
    } else {
  ...
  if (product_buy_now_price === null) {
    ...
    if (compare_price > product_buy_now_price) {
      if(!confirm('즉시 구매가보다 높은 가격에 구매할 수 없습니다. 즉시 구매 하시겠습니까?')) {
        return
      } else {
        location.href = `/product/auction_purchase/${auction_product_id}`
        compare_price = 'null'
